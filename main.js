...
app.post('/printInvoice', async (req, res) => {
  try {
    ...
    doc.fontSize(24).text('ABI METALS', { align: 'center' });
    doc.moveDown();
...
app.get('/getstocks', async (req, res) => {
  ...
  const stocks = await Stock.find({});
    // console.log(stocks);
    // Send the fetched stock data as JSON response
    res.json(stocks);
  } catch (error) {
...
app.get('/deletestocks', async (req, res) => {
    connectionToDB();
    try {
      const productName = req.name;
        const Stock = mongoose.models.Stock || mongoose.model('Stock', stockSchema);

        // Find the stock document with the given productName and delete it
        const deletedStock = await Stock.findOneAndDelete({ productName });

        if (!deletedStock) {
            // If no stock document was deleted, return an error message
