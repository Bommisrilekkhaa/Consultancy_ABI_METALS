...
async function connectionToDB(){
    ...
    mongoose.connect('mongodb+srv://chowdris21cse:mhCKlVV2Zw2wMPdp@cluster0.0wnr2pz.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0').then((data)=>{
...
async function AddStock(allStock) {
    ...
    try {
        ...
        for (const stockItem of allStock) {
            // Find the stock document with the same productName, if it exists
            const existingStock = await Stock.findOne({ productName: stockItem.productName });

            if (existingStock) {
                // If the product already exists, update its quantity
                await Stock.findOneAndUpdate(
                    { productName: stockItem.productName },
                    { $inc: { quantity: stockItem.quantity } } // Increment quantity
                );
            } else {
